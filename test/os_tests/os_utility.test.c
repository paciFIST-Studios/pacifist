...
START_TEST(fn_file_size__returns_minus_one__for_null_arg) {
    pf_set_error_suppressed();
    ck_assert(file_size(NULL, 0) == -1);
    pf_set_error_not_suppressed();
}

START_TEST(fn_file_size__returns_minus_one__for_bad_path_length) {
    pf_set_error_suppressed();
    char * path = "";
...
START_TEST(fn_file_size__writes_correct_error__for_null_arg) {
...
START_TEST(fn_file_size__writes_correct_error__for_bad_path_length) {
    // turn on error suppression, so it won't write to stdout
    pf_set_error_suppressed();
