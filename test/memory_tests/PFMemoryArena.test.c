...
START_TEST(fn_pf_memory_arena_set_bytes_to_zero__is_defined) {
    void (*fptr)(void* ptr, size_t const total_bytes) = & pf_allocator_memory_arena_set_bytes_to_zero;
...
START_TEST(fn_pf_memory_arena_set_bytes_to_zero__does_not_overflow_requested_size) {
    ...
    pf_allocator_memory_arena_set_bytes_to_zero(memory+quarter_size, half_size);
...
START_TEST(fn_pf_memory_arena_create_with_memory__is_defined) {
    PFAllocator_MemoryArena_t*(*fptr)(void*, size_t const) = &pf_allocator_memory_arena_create_with_memory;
    ck_assert_ptr_nonnull(fptr);
}
END_TEST
...
START_TEST(fn_pf_memory_arena_create_with_memory__uses_memory_to_create_new_arena_correctly) {
    ...
    PFAllocator_MemoryArena_t const * arena = pf_allocator_memory_arena_create_with_memory(memory_arena_start, one_half);
...
START_TEST(fn_pf_memory_arena_push_size__is_defined) {
    void*(*fptr)(PFAllocator_MemoryArena_t*, size_t const) = pf_allocator_memory_arena_push_size;
    ck_assert_ptr_nonnull(fptr);
}
END_TEST
...
START_TEST(fn_pf_memory_arena_push_size__returns_null__for_null_ptr_to_arena) {
    PF_SUPPRESS_ERRORS
    ck_assert_ptr_null(pf_allocator_memory_arena_push_size(NULL, 1));
...
START_TEST(fn_pf_memory_arena_push_size__returns_null__for_not_enough_memory) {
    ...
    PFAllocator_MemoryArena_t* arena = pf_allocator_memory_arena_create_with_memory(memory, memory_size);
    ck_assert_ptr_nonnull(arena);

    // allowed, b/c this is 16 of 32 free bytes
    void* alloc1 = pf_allocator_memory_arena_push_size(arena, 16);
    ck_assert_ptr_nonnull(arena);

    // not allowed, b/c there isn't this much left
    PF_SUPPRESS_ERRORS
    void* alloc2 = pf_allocator_memory_arena_push_size(arena, 17);
    PF_UNSUPPRESS_ERRORS
    ck_assert_ptr_null(alloc2);

    // not allowed, b/c there is exactly this much left
    PF_SUPPRESS_ERRORS
    void* alloc3 = pf_allocator_memory_arena_push_size(arena, 16);
    PF_UNSUPPRESS_ERRORS
    ck_assert_ptr_null(alloc3);

    // allowed b/c we can spare this much, and maybe make another allocation
    PF_SUPPRESS_ERRORS
    void* alloc4 = pf_allocator_memory_arena_push_size(arena, 8);
    PF_UNSUPPRESS_ERRORS
...
START_TEST(fn_pf_memory_arena_push_size__returns_null__for_zero_request) {
    ...
    PF_SUPPRESS_ERRORS
    ck_assert_ptr_null(pf_allocator_memory_arena_push_size(arena, 0));
    PF_UNSUPPRESS_ERRORS
...
START_TEST(fn_pf_memory_arena_push_size__writes_correct_error__for_not_enough_memory) {
    ...
    PF_SUPPRESS_ERRORS
    ck_assert_ptr_null(pf_allocator_memory_arena_push_size(arena, 16));
    PF_UNSUPPRESS_ERRORS
